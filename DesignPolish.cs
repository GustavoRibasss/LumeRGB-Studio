...
partial class ThebestRGB {
 internal const string ReleaseVersion="22.6.12";
 ...
 void LayoutDesign(){if(!designReady||designLayout)return;designLayout=true;main.SuspendLayout();side.SuspendLayout();try{
  ...
  if(powerButton!=null){powerButton.SetBounds(Math.Max(0,footer.ClientSize.Width-U(158)),U(10),U(142),U(34));if(pendingBadge!=null){pendingBadge.SetBounds(Math.Max(U(212),powerButton.Left-U(174)),U(12),U(158),U(30));pendingBadge.Region=new Region(RoundedRect(new Rectangle(0,0,pendingBadge.Width,pendingBadge.Height),U(15)));status.Width=Math.Max(U(100),pendingBadge.Left-status.Left-U(16));}else{status.Width=Math.Max(U(100),powerButton.Left-status.Left-U(16));}}stage.Invalidate(true);LayoutWindowChrome();UpdateScrollBar();
 }finally{side.ResumeLayout();main.ResumeLayout();designLayout=false;}}
 static System.Drawing.Drawing2D.GraphicsPath RoundedRect(Rectangle bounds,int radius){var path=new System.Drawing.Drawing2D.GraphicsPath();int d=radius*2;path.AddArc(bounds.X,bounds.Y,d,d,180,90);path.AddArc(bounds.Right-d,bounds.Y,d,d,270,90);path.AddArc(bounds.Right-d,bounds.Bottom-d,d,d,0,90);path.AddArc(bounds.X,bounds.Bottom-d,d,d,90,90);path.CloseFigure();return path;}
