...
class RestoreRegression{
 [STAThread]static void Main(){var flags=BindingFlags.Instance|BindingFlags.NonPublic;ProfileStore.FileName=Path.Combine(Path.GetTempPath(),"restore-test-"+Guid.NewGuid(),"profiles.json");using(var f=new LumeStudio()){
 var cards=(List<LightCard>)typeof(LumeStudio).GetField("cards",flags).GetValue(f);
 cards[0].State.R=21;cards[0].State.G=45;cards[0].State.B=180;cards[0].State.Brightness=37;
 typeof(LumeStudio).GetMethod("SaveRestorePoint",flags).Invoke(f,null);cards[0].State.R=255;
 typeof(LumeStudio).GetMethod("RestoreSavedSetup",flags).Invoke(f,null);
