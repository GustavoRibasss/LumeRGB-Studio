...
partial class ThebestRGB {
 ...
 void RestoreWindow(){var area=Screen.PrimaryScreen.WorkingArea;WindowState=FormWindowState.Normal;int width=Math.Min(Math.Max(Width,1180),area.Width),height=Math.Min(Math.Max(Height,760),area.Height);Bounds=new Rectangle(area.Left+(area.Width-width)/2,area.Top+(area.Height-height)/2,width,height);Show();Activate();BringToFront();LayoutDesign();}
