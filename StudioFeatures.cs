...
partial class ThebestRGB {
 ...
 void TrackSetup(){if(restoringSetup||undoSetup==null)return;if(colorRamp!=null&&colorRamp.Tone.ToArgb()!=global.ToArgb()){colorRamp.Tone=global;colorRamp.Invalidate();}var next=CaptureSetup();if(lastSetup!=null&&next.Key()!=lastSetup.Key()){if((DateTime.UtcNow-lastEdit).TotalMilliseconds>450)undoStates.Push(lastSetup);lastEdit=DateTime.UtcNow;}lastSetup=next;undoSetup.Enabled=undoStates.Count>0&&!busy;LocalBackup.BackupSetup(next);}
