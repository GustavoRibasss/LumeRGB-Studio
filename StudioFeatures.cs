...
partial class ThebestRGB {
 ...
 void InitializeFeatures(){
  ...
  presetButton=Button("Predefinições",surface);presetButton.Parent=main;presetButton.Click+=delegate{if(busy)return;var menu=new ContextMenuStrip{BackColor=surface,ForeColor=Color.White,ShowImageMargin=false,Renderer=new StudioMenuRenderer()};var fixedGroup=new ToolStripMenuItem("Cor fixa");fixedGroup.DropDown.BackColor=surface;fixedGroup.DropDown.ForeColor=Color.White;fixedGroup.DropDown.Renderer=new StudioMenuRenderer();string[] fixedNames={"Branco","Vermelho","Azul","Verde","Roxo","Laranja"};Color[] fixedColors={Color.White,Color.FromArgb(255,48,75),Color.FromArgb(75,184,255),Color.FromArgb(32,228,151),Color.FromArgb(160,96,255),Color.FromArgb(255,155,56)};for(int fi=0;fi<fixedNames.Length;fi++){Color chosen=fixedColors[fi];fixedGroup.DropDownItems.Add(fixedNames[fi],null,delegate{effectMode.SelectedIndex=0;SetGlobal(chosen);});}menu.Items.Add(fixedGroup);for(int mode=1;mode<EffectLibrary.Names.Length;mode++){if(!EffectLibrary.IsMainEffect(mode))continue;var presets=EffectPresets.ForMode(mode).ToArray();if(presets.Length==0)continue;var group=new ToolStripMenuItem(EffectLibrary.Names[mode]);group.DropDown.BackColor=surface;group.DropDown.ForeColor=Color.White;group.DropDown.Renderer=new StudioMenuRenderer();foreach(var p in presets){var chosen=p;group.DropDownItems.Add(p.Name,null,delegate{UsePreset(chosen);});}menu.Items.Add(group);}menu.Closed+=delegate{BeginInvoke(new Action(()=>menu.Dispose()));};menu.Show(presetButton,new Point(0,presetButton.Height));};
  ...
  tray=new NotifyIcon{Icon=LoadAppIcon(),Text="ThebestRGB",ContextMenuStrip=trayMenu,Visible=true};tray.DoubleClick+=delegate{RestoreWindow();};FormClosed+=delegate{tray.Visible=false;tray.Dispose();trayMenu.Dispose();};
 ...
 void HideToTray(){
  if(!Visible)return;
  // Hiding first avoids Windows briefly redrawing the taskbar item while the
  // window is still on screen. The next UI message then detaches it cleanly.
  Hide();
  BeginInvoke(new Action(delegate{if(!IsDisposed)ShowInTaskbar=false;}));
 }
 void RestoreWindow(){var area=Screen.PrimaryScreen.WorkingArea;ShowInTaskbar=true;WindowState=FormWindowState.Normal;int width=Math.Min(Math.Max(Width,1180),area.Width),height=Math.Min(Math.Max(Height,760),area.Height);Bounds=new Rectangle(area.Left+(area.Width-width)/2,area.Top+(area.Height-height)/2,width,height);Show();Activate();BringToFront();LayoutDesign();}
 void ToggleWindowFromTaskbar(){if(Visible&&WindowState!=FormWindowState.Minimized){HideToTray();return;}RestoreWindow();}
 ...
 void RefreshPendingBadge(){if(pendingBadge==null)return;if(!setupDirty){pendingBadge.Text="●  Iluminação aplicada";pendingBadge.ForeColor=Color.FromArgb(112,225,172);pendingBadge.BackColor=Color.FromArgb(19,45,32);}else if(autoApply){pendingBadge.Text="●  Aplicando…";pendingBadge.ForeColor=Color.FromArgb(255,210,118);pendingBadge.BackColor=Color.FromArgb(50,38,15);}else{pendingBadge.Text="●  Pendente";pendingBadge.ForeColor=Color.FromArgb(255,210,118);pendingBadge.BackColor=Color.FromArgb(50,38,15);}}
 void TrackSetup(){if(restoringSetup||undoSetup==null)return;if(colorRamp!=null&&!choosingTone&&colorRamp.Tone.ToArgb()!=global.ToArgb())colorRamp.ResetTone(global);var next=CaptureSetup();if(lastSetup!=null&&next.Key()!=lastSetup.Key()){if((DateTime.UtcNow-lastEdit).TotalMilliseconds>450)undoStates.Push(lastSetup);lastEdit=DateTime.UtcNow;}lastSetup=next;undoSetup.Enabled=undoStates.Count>0&&!busy;LocalBackup.BackupSetup(next);setupDirty=true;RefreshPendingBadge();QueueAutoApply();}
 ...
 void UsePreset(StudioPreset p){if(busy)return;restoringSetup=true;try{effectOptions[p.Mode]=p.Options.Copy();effectMode.SelectedIndex=p.Mode;}finally{restoringSetup=false;}lastEdit=DateTime.MinValue;TrackSetup();status.Text=p.Name+" preparada. Configure para visualizar ou aplique aos selecionados.";QueueAutoApply();}
