...
partial class LumeStudio {
 ...
 void InitializeFeatures(){
  ...
  presetButton=Button("Predefinições",surface);presetButton.Parent=main;presetButton.Click+=delegate{if(busy)return;var menu=new ContextMenuStrip{BackColor=surface,ForeColor=Color.White,Renderer=new StudioMenuRenderer()};foreach(var p in StudioPreset.All()){var item=menu.Items.Add(p.Name);item.Click+=delegate{UsePreset(p);};}menu.Show(presetButton,new Point(0,presetButton.Height));};
  ...
  trayMenu=new ContextMenuStrip{Renderer=new StudioMenuRenderer()};trayMenu.Items.Add("Abrir Lume",null,delegate{RestoreWindow();});trayMenu.Items.Add("Parar efeito",null,async delegate{if(!busy){SetBusy(true);try{await StopEffect();status.Text="Efeito parado.";}finally{SetBusy(false);}}});
  ...
  tray=new NotifyIcon{Icon=LoadAppIcon(),Text="Lume Studio",ContextMenuStrip=trayMenu,Visible=true};tray.DoubleClick+=delegate{RestoreWindow();};Resize+=delegate{if(WindowState==FormWindowState.Minimized&&!editorOpen)Hide();};FormClosed+=delegate{tray.Visible=false;tray.Dispose();trayMenu.Dispose();};
 ...
 void TrackSetup(){if(restoringSetup||undoSetup==null)return;var next=CaptureSetup();if(lastSetup!=null&&next.Key()!=lastSetup.Key()){if((DateTime.UtcNow-lastEdit).TotalMilliseconds>450)undoStates.Push(lastSetup);lastEdit=DateTime.UtcNow;}lastSetup=next;undoSetup.Enabled=undoStates.Count>0&&!busy;}
