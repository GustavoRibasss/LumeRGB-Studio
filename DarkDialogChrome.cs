...
static class DarkDialogChrome {
 ...
 public static void Attach(Form form,string title){
  ...
  form.Resize+=delegate{Layout(form,bar,caption,minButton,maxButton,closeButton,title);};form.Shown+=delegate{LumeStudio.ApplyDarkTitleBar(form);bar.BringToFront();};Layout(form,bar,caption,minButton,maxButton,closeButton,title);
 }
 static Button ChromeButton(string text,Color color,Control parent){var b=LumeStudio.Button(text,color);b.Font=new Font("Segoe UI",10);b.ForeColor=Color.FromArgb(225,226,235);b.TabStop=false;parent.Controls.Add(b);return b;}
