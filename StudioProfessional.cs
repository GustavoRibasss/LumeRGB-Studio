...
partial class LumeStudio {
 ...
 void InitializeProfessional(){
  ...
  profileList.DrawItem+=delegate(object sender,DrawItemEventArgs e){if(e.Index<0)return;using(var b=new SolidBrush((e.State&DrawItemState.Selected)!=0?Color.FromArgb(40,34,57):side.BackColor))e.Graphics.FillRectangle(b,e.Bounds);TextRenderer.DrawText(e.Graphics,profileList.Items[e.Index].ToString(),profileList.Font,new Rectangle(8,e.Bounds.Y,132,e.Bounds.Height),Color.White,TextFormatFlags.VerticalCenter|TextFormatFlags.EndEllipsis);};
  ...
  effectMode.Visible=false;modePicker=Button(EffectLibrary.Names[effectMode.SelectedIndex]+"   ▾",Color.FromArgb(31,32,41));modePicker.SetBounds(18,86,244,30);hero.Controls.Add(modePicker);modes=new ContextMenuStrip{BackColor=Color.FromArgb(31,32,41),ForeColor=Color.White,ShowImageMargin=false,Renderer=new StudioMenuRenderer()};for(int m=0;m<EffectLibrary.Names.Length;m++){int index=m;var item=modes.Items.Add(EffectLibrary.Names[m]);item.Click+=delegate{effectMode.SelectedIndex=index;};}modePicker.Click+=delegate{if(!busy)modes.Show(modePicker,new Point(0,modePicker.Height));};effectMode.SelectedIndexChanged+=delegate{modePicker.Text=effectMode.Text+"   ▾";};configure.SetBounds(18,124,244,32);pick.SetBounds(18,207,135,34);hex.SetBounds(172,217,90,24);hex.BackColor=surface;
  Color[] palette={Color.FromArgb(224,219,255),Color.FromArgb(80,220,240),Color.FromArgb(160,96,255),Color.FromArgb(255,80,132),Color.FromArgb(255,175,90),Color.White};for(int i=0;i<palette.Length;i++){Color c=palette[i];var b=Button("",c);b.SetBounds(18+i*41,251,32,20);hero.Controls.Add(b);b.Click+=delegate{if(!busy)SetGlobal(c);};help.SetToolTip(b,"#"+(c.ToArgb()&0xffffff).ToString("X6"));}
