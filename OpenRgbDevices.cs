...
class OpenRgbDevicesForm:Form {
 ...
 async void Scan(){if(!refresh.Enabled)return;refresh.Enabled=false;devices.Items.Clear();status.Text="Consultando o servidor OpenRGB…";try{var found=await Task.Run(()=>RamBridge.DiscoverAll());if(found.Count==0){devices.Items.Add("Nenhum dispositivo encontrado.");status.Text="Nenhum dispositivo foi exposto pelo OpenRGB. Inicie o servidor e tente novamente.";}else{foreach(var d in found)devices.Items.Add(d.name+"  •  "+(string.IsNullOrEmpty(d.location)?"local não informado":d.location)+"  •  "+d.leds+" LEDs"+(d.direct?"  •  Direct":""));status.Text=found.Count+" dispositivo(s) encontrado(s). Detecção concluída sem alterar cores.";}}catch(Exception ex){devices.Items.Add("OpenRGB não está disponível.");status.Text="Não foi possível consultar o OpenRGB: "+ex.Message;}finally{refresh.Enabled=true;}}
}
