...
partial class EffectEditor {
 ...
 void InitializeEditorFeatures(){
  ...
  var presets=LumeStudio.Button("Predefinição",Color.FromArgb(35,36,45));left.Controls.Add(presets);presets.SetBounds(26,125,150,28);presets.Click+=delegate{if(sending)return;var menu=new ContextMenuStrip{Renderer=new StudioMenuRenderer()};foreach(var p in StudioPreset.All()){if(p.Mode!=mode)continue;menu.Items.Add(p.Name,null,delegate{Result=p.Options.Copy();Speed=p.Speed;TrackEditor();BuildOptions();});}if(menu.Items.Count==0)menu.Items.Add("Padrão do efeito",null,delegate{Result=new EffectOptions();Speed=3;TrackEditor();BuildOptions();});menu.Show(presets,new Point(0,presets.Height));};
