...
partial class EffectEditor {
 ...
 void InitializeEditorFeatures(){
  undoEdit=LumeStudio.Button("Desfazer ajuste",Color.FromArgb(35,36,45));left.Controls.Add(undoEdit);undoEdit.SetBounds(192,125,150,28);undoEdit.Click+=delegate{if(sending||editorHistory.Count==0)return;var old=editorHistory.Pop();restoringEdit=true;Result=old.Item1.Copy();Speed=old.Item2;BuildOptions();restoringEdit=false;previousEdit=Result.Copy();previousSpeed=Speed;undoEdit.Enabled=editorHistory.Count>0;note.Text="Último ajuste desfeito na prévia.";};
var presets=LumeStudio.Button("Predefinição",Color.FromArgb(35,36,45));left.Controls.Add(presets);presets.SetBounds(26,125,150,28);presets.Click+=delegate{if(sending)return;var menu=new ContextMenuStrip{Renderer=new StudioMenuRenderer()};foreach(var p in EffectPresets.ForMode(mode)){if(p.Mode!=mode)continue;menu.Items.Add(p.Name,null,delegate{Result=p.Options.Copy();Speed=p.Speed;TrackEditor();BuildOptions();});}if(menu.Items.Count==0)menu.Items.Add("Padrão do efeito",null,delegate{Result=new EffectOptions();Speed=3;TrackEditor();BuildOptions();});menu.Show(presets,new Point(0,presets.Height));};
  applyDirect=LumeStudio.Button("Aplicar e fechar",Color.FromArgb(151,119,246));bottom.Controls.Add(applyDirect);applyDirect.SetBounds(bottom.Width-198,23,178,34);applyDirect.Anchor=AnchorStyles.Right|AnchorStyles.Top;applyDirect.Click+=async delegate{await ApplyAndClose();};
