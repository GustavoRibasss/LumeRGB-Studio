...
partial class EffectEditor:Form {
 ...
 void RefreshTabs(){int position=0;for(int i=0;i<tabs.Length;i++){bool visible=pages[i].Controls.Count>0;tabs[i].Visible=visible;if(visible){tabs[i].SetBounds(20+position++*147,68,139,34);}}if(!tabs[Math.Max(0,Math.Min(currentTab,tabs.Length-1))].Visible)currentTab=Enumerable.Range(0,tabs.Length).First(i=>tabs[i].Visible);} void BuildOptions(){
  ...
  if(IsHandleCreated){float factor=E(96)/96f;if(factor!=1)foreach(var page in pages)foreach(Control item in page.Controls)item.Scale(new SizeF(factor,factor));}RefreshTabs();SelectTab(currentTab);Arrange();
