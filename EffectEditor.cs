...
partial class EffectEditor:Form {
 ...
 void RefreshTabs(){int position=0;for(int i=0;i<tabs.Length;i++)if(availableTabs[i])tabs[i].SetBounds(20+position++*147,68,139,34);if(position==0){availableTabs[2]=true;position=1;}if(!availableTabs[Math.Max(0,Math.Min(currentTab,tabs.Length-1))])currentTab=Enumerable.Range(0,tabs.Length).FirstOrDefault(i=>availableTabs[i]);} void BuildOptions(){
