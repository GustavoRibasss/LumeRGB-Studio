...
partial class EffectEditor:Form {
 ...
 void BuildOptions(){
  ...
  AddColor(mode==16?"Pulsos":"Cor principal",mode==16?"Cor dos flashes que percorrem o setup.":"A cor que mais aparece no efeito.",Result.Foreground,v=>Result.Foreground=v);
  AddColor(mode==16?"Base":"Cor secundária",mode==16?"Tom suave que aparece entre os flashes.":"Uma segunda cor para combinar com a principal.",Result.Grid,v=>Result.Grid=v);
  ...
  if(mode==18){var follow=new StudioCheckBox{Text="Acompanhar luminosidade da tela",Checked=Result.FollowScreenBrightness,ForeColor=Color.White,Width=424,Height=32};pages[2].Controls.Add(follow);follow.CheckedChanged+=delegate{Result.FollowScreenBrightness=follow.Checked;TrackEditor();};TextBlock(2,"Ambilight unificado","A cor mais forte da cena é aplicada em todo o setup. Desmarcado: mantém cores fortes; tela preta preserva a última cor.");}
