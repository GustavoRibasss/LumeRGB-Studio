...
partial class EffectEditor:Form {
 ...
 void SelectTab(int index){if(index<0||index>=tabs.Length||!tabs[index].Visible)index=Enumerable.Range(0,tabs.Length).FirstOrDefault(i=>tabs[i].Visible);currentTab=index;for(int i=0;i<3;i++){pages[i].Visible=tabs[i].Visible&&i==index;tabs[i].BackColor=i==index?Color.FromArgb(70,54,107):Color.FromArgb(30,31,40);}}
 void RefreshTabs(){int position=0;for(int i=0;i<tabs.Length;i++)if(tabs[i].Visible)tabs[i].SetBounds(20+position++*147,68,139,34);if(position==0){tabs[2].Visible=true;position=1;}if(!tabs[Math.Max(0,Math.Min(currentTab,tabs.Length-1))].Visible)currentTab=Enumerable.Range(0,tabs.Length).FirstOrDefault(i=>tabs[i].Visible);} void BuildOptions(){
  ...
  if(IsHandleCreated){float factor=E(96)/96f;if(factor!=1)foreach(var page in pages)foreach(Control item in page.Controls)item.Scale(new SizeF(factor,factor));}tabs[0].Visible=screen||electric||colorDriven;tabs[1].Visible=!audio;tabs[2].Visible=!screen;RefreshTabs();SelectTab(currentTab);Arrange();
