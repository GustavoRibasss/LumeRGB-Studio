...
partial class EffectEditor:Form {
 ...
 public EffectEditor(int mode,EffectOptions initial,int speed,LightState[] states){
  ...
  for(int i=0;i<3;i++){int tab=i;tabs[i]=LumeStudio.Button(new[]{"Cores","Movimento","Intensidade"}[i],Color.FromArgb(30,31,40));tabs[i].SetBounds(20+i*147,68,139,34);right.Controls.Add(tabs[i]);tabs[i].Click+=delegate{SelectTab(tab);};pages[i]=new FlowLayoutPanel{Location=new Point(20,120),Size=new Size(440,450),FlowDirection=FlowDirection.TopDown,WrapContents=false,AutoScroll=false,Anchor=AnchorStyles.Top|AnchorStyles.Bottom|AnchorStyles.Left};right.Controls.Add(pages[i]);}
 ...
 void BuildOptions(){
  ...
  TextBlock(2,"Luz e acabamento","Ajuste a vivacidade e o brilho sem alterar as cores escolhidas.");
  if(mode==18){var follow=new StudioCheckBox{Text="Acompanhar luminosidade da tela",Checked=Result.FollowScreenBrightness,ForeColor=Color.White,Width=424,Height=32};pages[2].Controls.Add(follow);follow.CheckedChanged+=delegate{Result.FollowScreenBrightness=follow.Checked;TrackEditor();};TextBlock(2,"Brilho da tela","Desmarcado: só muda a cor; tela preta mantém a última cor.");}
