...
partial class EffectEditor:Form {
 ...
 void UpdatePreview(){for(int i=0;i<4;i++){Color c=EffectOptions.Sample(mode,states[i].Color,clock.Elapsed.TotalSeconds,Speed,i,Result);art[i].Light=c;art[i].Level=states[i].Brightness;art[i].Invalidate();rgb[i].Text="Brilho do dispositivo: "+states[i].Brightness+"%";}}
}
