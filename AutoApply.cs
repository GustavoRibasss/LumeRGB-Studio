...
partial class ThebestRGB {
 ...
 void InitializeAutoApply(){
  ...
  testSelected.Click+=delegate{autoApply=!autoApply;refresh();RefreshPendingBadge();if(!autoApply)autoApplyTimer.Stop();try{Directory.CreateDirectory(Path.GetDirectoryName(AutoApplyFile));File.WriteAllText(AutoApplyFile,autoApply?"1":"0");}catch(Exception ex){status.Text="Não foi possível salvar a preferência: "+ex.Message;}};
