...
class KeyboardBarForm:Form {
 ...
 public KeyboardBarForm(KeyboardBarSettings original,Func<KeyboardBarSettings,Task<string>> apply,Action<KeyboardBarSettings> prepare=null){
  ...
  color.Click+=delegate{using(var picker=new ColorDialog{Color=color.BackColor,FullOpen=true})if(picker.ShowDialog(this)==DialogResult.OK){settings.ColorValue=picker.Color.ToArgb()&0xffffff;color.BackColor=picker.Color;color.ForeColor=picker.Color.GetBrightness()>.6?Color.Black:Color.White;}};
