...
static class ScreenColors {
 ...
 static Color StrongestSceneColor(Bitmap image){
  ...
  int winner=0;for(int i=1;i<weight.Length;i++)if(weight[i]>weight[winner])winner=i;if(weight[winner]<=0)return neutralLight>=45?Color.White:Color.Black;if(neutralLight>=45&&weight[winner]<neutralLight*.20)return Color.White;Color source=Color.FromArgb((int)(red[winner]/weight[winner]),(int)(green[winner]/weight[winner]),(int)(blue[winner]/weight[winner]));float hue=source.GetHue();if(hue<24||hue>=336)return Color.Red;if(hue<48)return Color.FromArgb(255,128,0);if(hue<72)return Color.Yellow;if(hue<166)return Color.Lime;if(hue<211)return source.GetBrightness()>=.42f?Color.FromArgb(102,217,255):Color.FromArgb(0,220,255);if(hue<271)return Color.FromArgb(0,96,255);return Color.FromArgb(176,96,255);
 }
