...
InitializeEffects();InitializeEditor();InitializeProfessional();InitializeFeatures();InitializeMaxFeatures();InitializeKeyboardBar();InitializeUpgrade();InitializeDesign();InitializeScreenControls();RefreshProfiles();Resize+=delegate{LayoutMain();};Shown+=delegate{var area=Screen.PrimaryScreen.WorkingArea;int width=Math.Min(Math.Max(Width,1180),area.Width),height=Math.Min(Math.Max(Height,760),area.Height);Bounds=new Rectangle(area.Left+(area.Width-width)/2,area.Top+(area.Height-height)/2,width,height);LayoutMain();BeginInvoke(new Action(()=>LocalBackup.Initialize(this)));};LayoutMain();
 }
