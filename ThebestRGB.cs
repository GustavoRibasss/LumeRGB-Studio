...
class DeviceArt:Control {
 ...
 public DeviceArt(int kind){Kind=kind;SetStyle(ControlStyles.ResizeRedraw|ControlStyles.AllPaintingInWmPaint|ControlStyles.UserPaint,true);DoubleBuffered=true;BackColor=Color.FromArgb(17,21,31);Size=new Size(340,90);}
 protected override void OnPaint(PaintEventArgs e){base.OnPaint(e);var g=e.Graphics;g.Clear(BackColor);g.SmoothingMode=SmoothingMode.AntiAlias;float artScale=Math.Max(.1f,Math.Min(Width/260f,Height/96f));g.ScaleTransform(artScale,artScale);Color c=Color.FromArgb(Light.R*Level/100,Light.G*Level/100,Light.B*Level/100);float cx=Width/(2f*artScale);using(var pen=new Pen(Color.FromArgb(66,74,93),1.5f))using(var glow=new Pen(Color.FromArgb(45,c),9))using(var lit=new Pen(c,3))using(var fill=new SolidBrush(c)) {
