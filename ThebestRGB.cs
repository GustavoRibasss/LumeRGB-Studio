...
InitializeEffects();InitializeEditor();InitializeProfessional();InitializeFeatures();InitializeMaxFeatures();InitializeKeyboardBar();InitializeUpgrade();InitializeDesign();InitializeScreenControls();RefreshProfiles();Resize+=delegate{LayoutMain();};Shown+=delegate{LayoutMain();BeginInvoke(new Action(()=>LocalBackup.Initialize(this)));};LayoutMain();
 }
 void LayoutMain(){LayoutProfessional();}
 void SetGlobal(Color c){global=c;pick.BackColor=global;pick.ForeColor=global.GetBrightness()>.6?Color.Black:Color.White;hex.Text="#"+global.R.ToString("X2")+global.G.ToString("X2")+global.B.ToString("X2");if(colorRamp!=null){colorRamp.Tone=global;colorRamp.Invalidate();}foreach(var card in cards.Where(x=>x.State.Included)){card.State.R=global.R;card.State.G=global.G;card.State.B=global.B;card.RefreshState(false);card.Status.Text="Não aplicado";}status.Text="Cor preparada para os dispositivos selecionados.";TrackSetup();}
