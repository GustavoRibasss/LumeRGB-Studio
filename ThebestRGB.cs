...
// ThebestRGB Studio. Hardware adapters: ThebestRGB.cs, VisionGpu.cs, RamBridge.cs.
using System;
...
static class ProfileStore {
 public static string FileName=Path.Combine(BrandMigration.DataDirectory(),"profiles.json");
...
public static void Upgrade(List<LightProfile> all){if(all==null)return;foreach(var p in all){if(p==null)continue;if(p.Bar==null)p.Bar=new KeyboardBarSettings();if(p.MsiZones==null||!p.MsiZones.Valid())p.MsiZones=new MsiZoneSettings();if(p.Options!=null&&p.Options.Length>=17&&p.Options.Length<EffectLibrary.Names.Length&&p.Options.All(EffectOptions.Valid))p.Options=p.Options.Concat(Enumerable.Range(p.Options.Length,EffectLibrary.Names.Length-p.Options.Length).Select(i=>new EffectOptions())).ToArray();}} public static void Write(List<LightProfile> all){if(all.Count>100||all.Any(x=>!Valid(x)))throw new IOException("Perfil inválido.");Directory.CreateDirectory(Path.GetDirectoryName(FileName));string tmp=FileName+".tmp";File.WriteAllText(tmp,new JavaScriptSerializer().Serialize(all));if(File.Exists(FileName))File.Replace(tmp,FileName,null);else File.Move(tmp,FileName);LocalBackup.BackupFiles();}
}
class DeviceArt:Control {
 public KeyboardBarSettings BarPreview;public Color Light=Color.MediumPurple;public int Level=100;public int Kind;public bool Interactive,Selected;public string Caption="";bool hover;
 public DeviceArt(int kind){Kind=kind;SetStyle(ControlStyles.ResizeRedraw|ControlStyles.AllPaintingInWmPaint|ControlStyles.UserPaint,true);DoubleBuffered=true;BackColor=Color.FromArgb(17,21,31);Size=new Size(340,90);AccessibleRole=AccessibleRole.PushButton;}
 protected override void OnMouseEnter(EventArgs e){base.OnMouseEnter(e);if(Interactive){hover=true;Cursor=Cursors.Hand;Invalidate();}}
 protected override void OnMouseLeave(EventArgs e){base.OnMouseLeave(e);if(Interactive){hover=false;Cursor=Cursors.Default;Invalidate();}}
 protected override void OnPaint(PaintEventArgs e){base.OnPaint(e);var g=e.Graphics;g.Clear(BackColor);g.SmoothingMode=SmoothingMode.AntiAlias;if(Interactive){Color edge=Selected?Color.FromArgb(170,139,255):(hover?Color.FromArgb(90,113,126):Color.FromArgb(38,42,54));using(var p=new Pen(edge,Selected?2f:1f))g.DrawRectangle(p,1,1,Width-3,Height-3);if(!string.IsNullOrEmpty(Caption))using(var font=new Font("Segoe UI",7,FontStyle.Bold))using(var text=new SolidBrush(Selected?Color.FromArgb(222,211,255):Color.FromArgb(146,151,170)))g.DrawString(Caption,font,text,8,6);}
  float artScale=Math.Max(.1f,Math.Min(Width/260f,Height/96f));g.ScaleTransform(artScale,artScale);Color c=Color.FromArgb(Light.R*Level/100,Light.G*Level/100,Light.B*Level/100);float cx=Width/(2f*artScale);using(var pen=new Pen(Color.FromArgb(66,74,93),1.5f))using(var glow=new Pen(Color.FromArgb(45,c),9))using(var lit=new Pen(c,3))using(var fill=new SolidBrush(c)) {
  ...
  if(Kind==2){g.DrawRectangle(pen,cx-112,20,224,52);g.DrawLine(lit,cx-107,25,cx+107,25);for(int x=0;x<3;x++){g.DrawEllipse(pen,cx-92+x*62,31,33,33);for(int blade=0;blade<7;blade++){double angle=blade*Math.PI*2/7;float fx=cx-75.5f+x*62,fy=47.5f;g.DrawLine(pen,fx+(float)Math.Cos(angle)*5,fy+(float)Math.Sin(angle)*5,fx+(float)Math.Cos(angle+.45)*14,fy+(float)Math.Sin(angle+.45)*14);}g.FillEllipse(Brushes.DimGray,cx-80+x*62,43,9,9);}using(var f=new Font("Segoe UI",7,FontStyle.Bold))g.DrawString("GIGABYTE",f,fill,cx-30,76);}
  if(Kind==3){for(int y=0;y<2;y++){g.DrawRectangle(pen,cx-105,18+y*36,210,24);g.DrawLine(glow,cx-102,18+y*36,cx+102,18+y*36);g.DrawLine(lit,cx-102,18+y*36,cx+102,18+y*36);using(var f=new Font("Segoe UI",7,FontStyle.Bold))g.DrawString("VIPER",f,Brushes.Silver,cx-91,24+y*36);}}
 }}
}
class LightCard:Panel {
 ...
 public LightCard(int kind,LightState state){DoubleBuffered=true;State=state;BackColor=Color.FromArgb(23,28,40);Padding=new Padding(16);Size=new Size(395,244);
  ...
  var subtitle=new Label{Text=new[]{"AULA HERO 68 · teclas + barra LED","MSI B650M PROJECT ZERO · 3 conectores ARGB","GIGABYTE RTX 3080 · logo RGB","VIPER 7000MHz branca · 2 módulos RGB"}[kind],ForeColor=ThebestRGB.Muted,AutoSize=true,Location=new Point(17,42),Font=new Font("Segoe UI",8)};Controls.Add(subtitle);
  ...
  Pick=ThebestRGB.Button("Cor",Color.FromArgb(44,51,69));Pick.SetBounds(16,199,75,30);Controls.Add(Pick);
  Apply=ThebestRGB.Button("Aplicar",Color.FromArgb(44,51,69));Apply.SetBounds(100,199,92,30);Controls.Add(Apply);
  Test=ThebestRGB.Button("Testar",Color.FromArgb(44,51,69));Test.SetBounds(196,199,74,30);Controls.Add(Test);
  Status=new Label{Text="Pronto",ForeColor=ThebestRGB.Muted,AutoSize=true,Location=new Point(209,207),Font=new Font("Segoe UI",8)};Controls.Add(Status);
  ...
  Pick.Click+=delegate{using(var d=new SavedColorDialog{Color=State.Color,FullOpen=true})if(d.ShowDialog()==DialogResult.OK){Color c=d.Color;State.R=c.R;State.G=c.G;State.B=c.B;RefreshState(false);Status.Text="Não aplicado";}};
...
partial class ThebestRGB:Form {
 ...
 public ThebestRGB(){
  ...
  main=new DarkScrollPanel{Dock=DockStyle.Fill,AutoScroll=false,Padding=new Padding(26)};Controls.Add(main);main.BringToFront();
  ...
  pick=Button("Escolher cor",global);pick.ForeColor=Color.Black;pick.SetBounds(18,41,123,32);hero.Controls.Add(pick);pick.Click+=delegate{using(var d=new SavedColorDialog{Color=global,FullOpen=true})if(d.ShowDialog()==DialogResult.OK)SetGlobal(d.Color);};
...
masterValue=Label("100%",10,Color.White,489,48);hero.Controls.Add(masterValue);master.ValueChanged+=delegate{masterValue.Text=master.Value+"%";foreach(var c in cards.Where(c=>c.State.Included)){c.State.Brightness=master.Value;c.RefreshState(true);c.Status.Text="Não aplicado";}};
  ...
  status=new Label{ForeColor=Muted,Font=new Font("Segoe UI",9),Text="Tudo pronto. Clique em Aplicar iluminação para usar as cores da prévia.",AutoSize=false};main.Controls.Add(status);
  try{profiles=ProfileStore.Read();}catch(Exception ex){profilesWritable=false;status.Text=ex.Message;}
InitializeEffects();InitializeEditor();InitializeProfessional();InitializeFeatures();InitializeMaxFeatures();InitializeKeyboardBar();InitializeUpgrade();InitializeDesign();InitializeScreenControls();StartMobileRemote();FormClosed+=delegate{StopMobileRemote();};RefreshProfiles();Resize+=delegate{LayoutMain();};Shown+=delegate{var area=Screen.PrimaryScreen.WorkingArea;int width=Math.Min(Math.Max(Width,1180),area.Width),height=Math.Min(Math.Max(Height,760),area.Height);Bounds=new Rectangle(area.Left+(area.Width-width)/2,area.Top+(area.Height-height)/2,width,height);LayoutMain();BeginInvoke(new Action(()=>LocalBackup.Initialize(this)));};LayoutMain();
 }
...
void SetGlobal(Color c){global=c;pick.BackColor=global;pick.ForeColor=global.GetBrightness()>.6?Color.Black:Color.White;hex.Text="#"+global.R.ToString("X2")+global.G.ToString("X2")+global.B.ToString("X2");if(colorRamp!=null&&!choosingTone)colorRamp.ResetTone(global);foreach(var card in cards.Where(x=>x.State.Included)){card.State.R=global.R;card.State.G=global.G;card.State.B=global.B;card.RefreshState(false);card.Status.Text="Não aplicado";}status.Text="Cor preparada para os dispositivos selecionados.";TrackSetup();if(!choosingTone)RememberColor(c);QueueAutoApply();}
 ...
 void SetBusy(bool value){busy=value;bool locked=value||blackoutActive;if(configure!=null)configure.Enabled=!locked;if(effectMode!=null){effectMode.Enabled=effectSpeed.Enabled=!locked;effectStop.Enabled=!locked&&!effectTask.IsCompleted;}all.Enabled=activate.Enabled=save.Enabled=load.Enabled=remove.Enabled=pick.Enabled=hex.Enabled=master.Enabled=profileList.Enabled=profileName.Enabled=!locked;if(colorRamp!=null)colorRamp.Enabled=!locked;foreach(var button in frequentButtons)button.Enabled=!locked;foreach(var c in cards){c.Enabled=!locked;c.Art.Enabled=!locked;c.Art.Interactive=!locked;c.Art.Invalidate();}if(selectAll!=null)selectAll.Enabled=!locked;Cursor=value?Cursors.WaitCursor:Cursors.Default;UpdateEffectControls();if(verifyDevices!=null){verifyDevices.Enabled=presetButton.Enabled=!locked;undoSetup.Enabled=!locked&&undoStates.Count>0;}UpdateMaxState();}
