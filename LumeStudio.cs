...
partial class LumeStudio:Form {
 ...
 public LumeStudio(){
  ...
  InitializeEffects();InitializeEditor();InitializeProfessional();InitializeFeatures();InitializeMaxFeatures();InitializeKeyboardBar();InitializeUpgrade();InitializeDesign();RefreshProfiles();Resize+=delegate{LayoutMain();};Shown+=delegate{LayoutMain();};Shown+=async delegate{try{await Task.Run(()=>RamBridge.Start());cards[3].Status.Text="Disponível";status.Text="Memória ativada automaticamente.";}catch{cards[3].Status.Text="Verificar acesso";}};LayoutMain();
 }
