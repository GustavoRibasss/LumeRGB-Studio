...
partial class LumeStudio:Form {
 ...
 static Icon LoadAppIcon(){try{using(var stream=typeof(LumeStudio).Assembly.GetManifestResourceStream("ThebestRGB.ico")){if(stream!=null)using(var icon=new Icon(stream))return (Icon)icon.Clone();}string path=Path.Combine(AppDomain.CurrentDomain.BaseDirectory,"LumeStudio.ico");if(File.Exists(path))return new Icon(path);}catch{}return SystemIcons.Application;}
