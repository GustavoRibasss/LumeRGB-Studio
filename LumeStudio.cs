...
public class MsiZoneSettings {
 ...
 public MsiZoneSettings Copy(){return (MsiZoneSettings)MemberwiseClone();}
 ...
 public Color[] Resolve(Color fallback){var zones=new[]{fallback,fallback,fallback};if(!Split)return zones;int fans=Math.Max(0,Math.Min(2,FansChannel)),water=Math.Max(0,Math.Min(2,WaterChannel));if(fans==water)water=(fans+1)%3;zones[fans]=FromRgb(Fans);zones[water]=FromRgb(WaterCooler);for(int i=0;i<3;i++)if(i!=fans&&i!=water){zones[i]=FromRgb(Auxiliary);break;}return zones;}
}
...
class DeviceArt:Control {
 ...
 protected override void OnPaint(PaintEventArgs e){base.OnPaint(e);var g=e.Graphics;g.SmoothingMode=SmoothingMode.AntiAlias;float artScale=Math.Max(.1f,Math.Min(g.DpiX/96f,Height/90f));g.ScaleTransform(artScale,artScale);Color c=Color.FromArgb(Light.R*Level/100,Light.G*Level/100,Light.B*Level/100);float cx=Width/(2f*artScale);using(var pen=new Pen(Color.FromArgb(66,74,93),1.5f))using(var glow=new Pen(Color.FromArgb(45,c),9))using(var lit=new Pen(c,3))using(var fill=new SolidBrush(c)) {
  ...
  if(Kind==1){g.DrawRectangle(pen,cx-60,9,120,72);g.DrawLine(pen,cx+37,9,cx+37,81);for(int y=0;y<3;y++){g.DrawEllipse(glow,cx-35,15+y*21,16,16);g.DrawEllipse(lit,cx-35,15+y*21,16,16);}g.DrawRectangle(lit,cx-5,24,30,4);g.DrawRectangle(pen,cx-5,40,32,26);}
...
partial class LumeStudio:Form {
 ...
 public LumeStudio(){
  ...
  InitializeEffects();InitializeEditor();InitializeProfessional();InitializeFeatures();InitializeMaxFeatures();InitializeKeyboardBar();InitializeUpgrade();RefreshProfiles();Resize+=delegate{LayoutMain();};Shown+=delegate{LayoutMain();};LayoutMain();
 }
 ...
 void SendStaticFrame(int index,Color c,int brightness){SendBalancedStaticFrame(index,c,brightness,Calibration.Values[index]);} void SendBalancedStaticFrame(int index,Color c,int brightness,ColorBalance balance){c=balance.Apply(c);int level=(brightness+2)/5;switch(index){case 0:Hid.Apply(c,level);break;case 1:if(msiZones!=null&&msiZones.Split){var zones=msiZones.Resolve(c);for(int i=0;i<zones.Length;i++)zones[i]=balance.Apply(zones[i]);Hid.ApplyMsi(zones,(brightness+5)/10);}else Hid.ApplyMsi(c,(brightness+5)/10);break;case 2:VisionGpu.Apply(c,(brightness*99+50)/100);break;case 3:RamBridge.Apply(c,level);break;}}
