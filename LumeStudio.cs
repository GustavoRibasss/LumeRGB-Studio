...
class DeviceArt:Control {
 ...
 static Image[] photos;
 static Image[] Photos(){if(photos!=null)return photos;photos=new Image[4];try{var asm=typeof(DeviceArt).Assembly;string[] names={null,"Hardware.MsiB650M.png","Hardware.GigabyteRtx3080.png","Hardware.ViperRam.png"};for(int i=1;i<4;i++){using(var s=asm.GetManifestResourceStream(names[i]))if(s!=null)photos[i]=Image.FromStream(s);}}catch{}return photos;}
 public DeviceArt(int kind){Kind=kind;DoubleBuffered=true;BackColor=Color.FromArgb(17,21,31);Size=new Size(340,90);}
 protected override void OnPaint(PaintEventArgs e){base.OnPaint(e);var g=e.Graphics;g.SmoothingMode=SmoothingMode.AntiAlias;var photo=Photos()[Kind];if(photo!=null){var box=new Rectangle(5,4,Width-10,Height-8);g.FillRectangle(Brushes.White,box);float scale=Math.Min((float)box.Width/photo.Width,(float)box.Height/photo.Height);var dest=new Rectangle(box.X+(box.Width-(int)(photo.Width*scale))/2,box.Y+(box.Height-(int)(photo.Height*scale))/2,(int)(photo.Width*scale),(int)(photo.Height*scale));g.DrawImage(photo,dest);using(var overlay=new SolidBrush(Color.FromArgb(Math.Max(0,90-Level),0,0,0)))g.FillRectangle(overlay,box);return;}g.SmoothingMode=SmoothingMode.AntiAlias;float artScale=Math.Max(.1f,Math.Min(g.DpiX/96f,Height/90f));g.ScaleTransform(artScale,artScale);Color c=Color.FromArgb(Light.R*Level/100,Light.G*Level/100,Light.B*Level/100);float cx=Width/(2f*artScale);using(var pen=new Pen(Color.FromArgb(66,74,93),1.5f))using(var glow=new Pen(Color.FromArgb(45,c),9))using(var lit=new Pen(c,3))using(var fill=new SolidBrush(c)) {
