...
class ColorRampPreview:Control {
 ...
 protected override void OnMouseUp(MouseEventArgs e){base.OnMouseUp(e);Capture=false;}
 protected override void OnPaint(PaintEventArgs e){base.OnPaint(e);var g=e.Graphics;var rect=new Rectangle(0,0,Math.Max(1,Width-1),Math.Max(1,Height-1));using(var left=new LinearGradientBrush(new Rectangle(0,0,Math.Max(1,Width/2),Height),Scale(Tone,.25),Tone,LinearGradientMode.Horizontal))g.FillRectangle(left,0,0,Math.Max(1,Width/2),Height);using(var right=new LinearGradientBrush(new Rectangle(Width/2,0,Math.Max(1,Width-Width/2),Height),Tone,Scale(Tone,.10),LinearGradientMode.Horizontal))g.FillRectangle(right,Width/2,0,Math.Max(1,Width-Width/2),Height);using(var border=new Pen(Color.FromArgb(81,82,98)))g.DrawRectangle(border,rect);int x=(int)Math.Round((Width-1)*value/100.0);using(var marker=new Pen(Color.White,2))g.DrawLine(marker,x,1,x,Height-2);}
}
