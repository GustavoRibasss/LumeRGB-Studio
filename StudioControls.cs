...
class StudioButton:Button {
 protected virtual Rectangle TextBounds {get{return new Rectangle(0,0,Width-1,Height-1);}}
 bool hover;
 ...
 protected override void OnPaint(PaintEventArgs e){var g=e.Graphics;g.SmoothingMode=SmoothingMode.AntiAlias;g.Clear(Parent==null?Color.FromArgb(15,16,20):Parent.BackColor);var rect=new Rectangle(0,0,Width-1,Height-1);using(var path=new GraphicsPath()){int r=12;path.AddArc(rect.Left,rect.Top,r,r,180,90);path.AddArc(rect.Right-r,rect.Top,r,r,270,90);path.AddArc(rect.Right-r,rect.Bottom-r,r,r,0,90);path.AddArc(rect.Left,rect.Bottom-r,r,r,90,90);path.CloseFigure();Color c=Enabled?BackColor:Color.FromArgb(28,29,35);if(Enabled&&hoverBlend>0)c=ControlPaint.Light(c,.12f*hoverBlend);using(var b=new SolidBrush(c))g.FillPath(b,path);using(var p=new Pen(Color.FromArgb(Enabled?50:34,52,63)))g.DrawPath(p,path);}TextRenderer.DrawText(g,Text,Font,TextBounds,Enabled?ForeColor:Color.FromArgb(105,108,122),TextFormatFlags.HorizontalCenter|TextFormatFlags.VerticalCenter|TextFormatFlags.EndEllipsis);if(Focused)ControlPaint.DrawFocusRectangle(g,new Rectangle(4,4,Width-9,Height-9));}
}
class ColorRampPreview:Control {
 public Color SelectedTone{get{return Spectrum(Value/100.0);}}
 static Color Spectrum(double position){double hue=position*6;int sector=(int)hue;int rise=(int)Math.Round((hue-sector)*255),fall=255-rise;switch(sector%6){case 0:return Color.FromArgb(255,rise,0);case 1:return Color.FromArgb(fall,255,0);case 2:return Color.FromArgb(0,255,rise);case 3:return Color.FromArgb(0,fall,255);case 4:return Color.FromArgb(rise,0,255);default:return Color.FromArgb(255,0,fall);}}
 public void ResetTone(Color color){Tone=color;value=(int)Math.Round(color.GetHue()/360.0*100);Invalidate();}
 public Color Tone=Color.White;int value=100;public int Value{get{return value;}set{int next=Math.Max(0,Math.Min(100,value));if(next==this.value)return;this.value=next;Invalidate();if(ValueChanged!=null)ValueChanged(this,EventArgs.Empty);}}public event EventHandler ValueChanged;
 public ColorRampPreview(){SetStyle(ControlStyles.UserPaint|ControlStyles.AllPaintingInWmPaint|ControlStyles.OptimizedDoubleBuffer|ControlStyles.ResizeRedraw|ControlStyles.Selectable,true);Height=14;Cursor=Cursors.Hand;TabStop=true;AccessibleRole=AccessibleRole.Slider;}
 static Color Scale(Color color,double amount){return Color.FromArgb((int)Math.Round(color.R*amount),(int)Math.Round(color.G*amount),(int)Math.Round(color.B*amount));}
 static Color Lighten(Color color,double amount){return Color.FromArgb((int)Math.Round(color.R+(255-color.R)*amount),(int)Math.Round(color.G+(255-color.G)*amount),(int)Math.Round(color.B+(255-color.B)*amount));}
 void SetFromX(int x){Value=(int)Math.Round(Math.Max(0,Math.Min(1,x/(double)Math.Max(1,Width-1)))*100);}
 ...
 protected override void OnMouseMove(MouseEventArgs e){base.OnMouseMove(e);if(Capture&&e.Button==MouseButtons.Left)SetFromX(e.X);}
 protected override void OnMouseUp(MouseEventArgs e){base.OnMouseUp(e);Capture=false;}
 protected override void OnPaint(PaintEventArgs e){base.OnPaint(e);for(int x=0;x<Width;x++)using(var pen=new Pen(Spectrum(x/(double)Math.Max(1,Width-1))))e.Graphics.DrawLine(pen,x,0,x,Height-1);using(var border=new Pen(Color.FromArgb(81,82,98)))e.Graphics.DrawRectangle(border,0,0,Math.Max(1,Width-1),Math.Max(1,Height-1));int marker=(int)Math.Round((Width-1)*Value/100.0);using(var dark=new Pen(Color.Black,4))e.Graphics.DrawLine(dark,marker,1,marker,Height-2);using(var light=new Pen(Color.White,2))e.Graphics.DrawLine(light,marker,1,marker,Height-2);}
}
class SetupStage:Panel {
 ...
 protected override void OnPaint(PaintEventArgs e){base.OnPaint(e);var g=e.Graphics;g.SmoothingMode=SmoothingMode.AntiAlias;using(var pen=new Pen(Color.FromArgb(34,36,46)))g.DrawRectangle(pen,0,0,Width-1,Height-1);using(var font=new Font("Segoe UI",8))using(var b=new SolidBrush(Color.FromArgb(137,139,155)))g.DrawString("EDITOR VISUAL DO SETUP",font,b,18,15);using(var font=new Font("Segoe UI",8))using(var b=new SolidBrush(Color.FromArgb(100,104,122)))g.DrawString("Clique para selecionar • botão direito para definir uma cor individual",font,b,18,Height-26);}
}
